      ...
      <div className="container mx-auto px-6">
        {(props.title || props.subtitle) && (
          <div className={`text-${s.textAlign} mb-12`}>
            {props.subtitle && <span className={`inline-block px-4 py-2 bg-primary/10 text-primary rounded-full text-sm font-medium mb-4 ${s.subtitleTransform} font-body-dynamic`}>{props.subtitle}</span>}
            {props.title && <h2 className={`${s.titleSize()} ${s.titleWeight} ${s.titleColor} font-heading-dynamic`}>{props.title}</h2>}
          </div>
        )}
          ...
          {plans.map((plan, index) => {
            ...
            return (
              <div key={index} className={`relative rounded-2xl p-8 ${plan.popular ? 'bg-primary text-primary-foreground shadow-2xl scale-105' : 'bg-card text-card-foreground shadow-lg'}`}>
                {plan.popular && <div className="absolute -top-4 left-1/2 -translate-x-1/2"><span className="bg-accent text-accent-foreground px-4 py-1 rounded-full text-sm font-medium font-body-dynamic">Popüler</span></div>}
                <div className="text-center mb-6">
                  <h3 className="text-xl font-bold mb-2 font-heading-dynamic">{plan.name}</h3>
                  <div className="flex items-baseline justify-center gap-1">
                    <span className="text-4xl font-bold font-heading-dynamic">{plan.price}</span>
                    {plan.period && <span className={`${plan.popular ? 'text-primary-foreground/70' : 'text-muted-foreground'} font-body-dynamic`}>/{plan.period}</span>}
                  </div>
                </div>
                <ul className="space-y-3 mb-8">
                  {featureList.map((feature: string, fIndex: number) => (
                    <li key={fIndex} className="flex items-center gap-3">
                      ...
                      <span className={`${plan.popular ? '' : 'text-muted-foreground'} font-body-dynamic`}>{feature}</span>
                    </li>
                  ))}
                </ul>
                <a href={isEditing ? "#" : "#contact"} className={`block w-full py-3 px-6 rounded-lg font-medium text-center transition-colors font-body-dynamic ${plan.popular ? 'bg-primary-foreground text-primary hover:bg-primary-foreground/90' : 'bg-primary text-primary-foreground hover:bg-primary/90'}`}>{plan.buttonText || "Başla"}</a>
              </div>
            );
          })}
        </div>
